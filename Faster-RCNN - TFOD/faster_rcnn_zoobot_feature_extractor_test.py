...
"""Tests for models.faster_rcnn_zoobot_feature_extractor."""
import unittest
import tensorflow.compat.v1 as tf

from object_detection.models import faster_rcnn_zoobot_feature_extractor as frcnn_res
from object_detection.utils import tf_version


@unittest.skipIf(tf_version.is_tf1(), 'Skipping TF2.X only test.')
class FasterRcnnZoobotFeatureExtractorTest(tf.test.TestCase):

  def _build_feature_extractor(self):
    return frcnn_res.FasterRCNNZoobotFeatureExtractor(
        is_training=False,
  ...
  def test_extract_box_classifier_features_returns_expected_size(self):
    ...
    proposal_feature_maps = tf.random_uniform(
        [3, 7, 7, 1152], maxval=255, dtype=tf.float32)
    ...
    self.assertAllEqual(features_shape.numpy(), [3, 7, 7, 1280])
