...
def main():
    ...
    parser.add_argument('--model', dest='model_id', help='Key from Model dictionary', type=str)
    ...
    for run in range(args.run_from, args.run_to, 1):
        ...
        tb_log_dir = model_dict[args.model_id]['log_dir'] + 'run={}/'.format(run) + 'train'
        writer = SummaryWriter(log_dir=tb_log_dir)
         # get the model
        frcnn_model = get_model(
            model_type=model_dict[args.model_id]['model_type'],
            num_classes=NUM_CLASSES,
            trainable_layers=model_dict[args.model_id]['trainable_layers']
        )
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            if (epoch+1) % 20 == 0:
                model_save_path = model_dict[args.model_id]['log_dir'] + 'run={}/'.format(run) + model_dict[args.model_id]['model_name'] + '_{}.pth'.format(epoch+1)
